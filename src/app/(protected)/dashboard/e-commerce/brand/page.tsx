...
import { useBrands } from "@/components/brand/useBrands";
import { useUpdateBrand } from "@/components/brand/useUpdateBrand";
import { useDeleteBrand } from "@/components/brand/useDeleteBrand";
import { useAddBrand } from "@/components/brand/useCreateBrand";
...
export default function BrandPage() {
  ...
  const { updateBrand, isPending: isUpdating } = useUpdateBrand();
  const { deleteBrand, isPending: isDeleting } = useDeleteBrand();
  const { addBrand, isPending: isAdding } = useAddBrand();
  ...
  const openEditModal = (brand: Brand) => {
    ...
    setEditAr(brand.name_ar);
    setEditEn(brand.name_en);
  ...
  const openDeleteModal = (brand: Brand) => setDeletingBrand(brand);
  const closeDeleteModal = () => setDeletingBrand(null);
  ...
  return (
    <>
      <div className="mb-[25px] md:flex items-center justify-between">
        <h5 className="!mb-0">البراندات</h5>
            ...
            <div className="bg-white dark:bg-[#0c1427] rounded-xl shadow-2xl p-8 w-full max-w-md mx-4">
              ...
              <div className="flex justify-end gap-4">
                ...
                <button
                  ...
                  className="px-6 py-3 rounded-lg bg-primary-600 hover:bg-primary-700 text-white font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isAdding ? "جاري الإضافة..." : "إضافة البراند"}
                </button>
              </div>
            </div>
          </div>
